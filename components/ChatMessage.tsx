...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
        ...
        {message.attachments.map((att, index) => (
          <div key={index} className="relative group">
            {att.type === 'image' ? (
              ...
              <div className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-stone-100 to-stone-50 rounded-xl border border-stone-200 shadow-sm">
                <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
                  <svg className="w-4 h-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                  </svg>
                </div>
                <span className="text-xs font-medium text-neutral-600">Voice Message</span>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center w-28 h-28 bg-gradient-to-br from-stone-50 to-stone-100 rounded-xl border border-stone-200 p-3 text-center shadow-sm group-hover:shadow-md transition-shadow">
                ...
                <span className="text-[10px] text-stone-500 truncate w-full font-medium">{att.name || 'Document'}</span>
              </div>
            )}
          </div>
        ))}
  ...
  return (
    <div className={`flex w-full mb-3 animate-slide-up ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`flex ${isUser ? 'max-w-[85%] md:max-w-[75%] flex-row-reverse' : 'w-full max-w-full px-2'} items-start gap-2.5`}>

        {/* Avatar */}
        <div className={`flex-shrink-0 h-8 w-8 rounded-lg flex items-center justify-center text-xs font-bold shadow-sm overflow-hidden mt-0.5 ${isUser
          ? 'bg-gradient-to-br from-neutral-800 to-neutral-700 text-white'
          : 'bg-white border border-stone-100 hidden' // Hide bot avatar for document feel, or keep it subtle
          }`}>
          {isUser ? (
        ...
        <div className={`flex flex-col flex-1 ${isUser ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`relative group text-sm md:text-base leading-relaxed ${isUser
            ? 'px-4 py-2.5 bg-gradient-to-br from-emerald-800 to-emerald-700 text-white rounded-2xl rounded-tr-sm shadow-md shadow-emerald-500/10'
            : isError
              ? 'px-4 py-3 bg-gradient-to-br from-red-50 to-rose-50 border border-red-100 text-red-800 rounded-2xl rounded-tl-sm shadow-sm'
              : 'w-full bg-transparent p-0 text-neutral-800' // Bot: Full width, transparent, no padding/box
            }`}>
            ...
            {isError ? (
              ...
              <div className={`markdown-body prose prose-sm max-w-none prose-p:my-2 prose-headings:mt-3 prose-headings:mb-1.5 prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5 ${isUser ? 'prose-invert' : 'prose-slate prose-p:leading-relaxed prose-li:leading-relaxed'}`}>
                <ReactMarkdown>
                ...
                {message.text.includes("*This is a limited free search. Please recharge your account for a robust and complete response.*") && (
                  <div className="mt-4 pt-3 border-t border-neutral-200">
                    <p className="text-neutral-500 text-xs font-semibold italic flex items-center gap-2">
            ...
            {!isUser && !isError && message.legalCitations && message.legalCitations.length > 0 && (
              <div className="mt-3 flex flex-wrap items-center gap-2 animate-fade-in">
                <span className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">{t('legalSources')}:</span>
                {message.legalCitations.map((c, idx) => {
                  ...
                  return c.source_url ? (
                    ...
                    <span key={idx} title={c.title || label}
                      className="inline-flex items-center gap-1.5 text-xs font-medium text-neutral-600 bg-stone-100 border border-stone-200 px-2.5 py-1 rounded-full">
                      {inner}
                    </span>
                  );
                })}
              ...
              <div className="mt-2 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${isCopied
                    ...
                    : 'bg-white text-neutral-400 border-stone-200 hover:text-emerald-600 hover:border-emerald-300 shadow-sm'}`}
                >
                  {isCopied ? (
                    <>
            ...
            {/* Web grounding sources are intentionally NOT shown to users — the
                answer stands on its own; raw web links (forums, social media) can
                mislead in a legal context. Kept in the payload for internal use. */}
