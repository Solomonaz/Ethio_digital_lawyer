...
import { loginUser, registerUser, completeRegistration, loginWithGoogle, requestVerificationCode, verifyPhoneCode } from '../services/storageService';
...
const AuthModal: React.FC<AuthModalProps> = ({ onLogin }) => {
  ...
  const [view, setView] = useState<'login' | 'signup' | 'verify' | 'collect-phone' | 'registration-success'>('login');
  ...
  const [googlePendingUser, setGooglePendingUser] = useState<User | null>(null); // Only for Google login flow
  const [error, setError] = useState('');
  const [successMessage, setSuccessMessage] = useState(''); // For showing success after registration
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'login') {
        ...
        // Phase 1: Submit registration, get verification code
        const result = await registerUser(name, email, phoneNumber, password);
        if (result.dev_code) {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      if (googlePendingUser) {
        // Google user verifying phone number (already registered)
        await verifyPhoneCode(phoneNumber, verificationCode);
        onLogin({ ...googlePendingUser, is_verified: true });
      } else {
        // Normal registration: Phase 2 - Verify code and complete registration
        await completeRegistration(phoneNumber, verificationCode);
        // Show success and redirect to login
        setSuccessMessage(t('registrationSuccess') || 'Registration successful! Please login with your credentials.');
        setView('login');
        // Clear form fields
        setName('');
        setPhoneNumber('');
  ...
  const handleResendCode = async () => {
    ...
    try {
      let result;
      if (googlePendingUser) {
        // Google user - use existing verification endpoint
        result = await requestVerificationCode(phoneNumber);
      } else {
        // Normal registration - re-submit registration to get new code
        result = await registerUser(name, email, phoneNumber, password);
      }
      if (result.dev_code) {
        setDevCode(result.dev_code);
      }
              ...
              {view === 'collect-phone' ? (
                ...
                <>
                  ...
                  {successMessage && view === 'login' && (
                    <div className="mb-5 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className="ml-auto flex-shrink-0 text-emerald-600 hover:text-emerald-800"
                        >
