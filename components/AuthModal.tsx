...
const AuthModal: React.FC<AuthModalProps> = ({ onLogin, initialView = 'login', onBack }) => {
  ...
  // Client-side password policy for signup: at least 8 chars, with a letter and a number.
        ...
        <div className="relative transform overflow-hidden rounded-3xl bg-white shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-4xl lg:flex">
          ...
          <div className="flex flex-1 flex-col justify-center bg-white lg:w-7/12">
            ...
            <div className="p-6 sm:p-8">
              <button onClick={handleGoogleLogin} disabled={isLoading} className="w-full flex items-center justify-center gap-2.5 h-11 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all mb-5 shadow-sm disabled:opacity-60">
              ...
              <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <div>
                  ...
                  {view === 'signup' && <p className="text-[11px] text-slate-400 mt-1">{t('passwordPolicy')}</p>}
                </div>
