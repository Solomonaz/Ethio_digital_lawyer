...
const AuthModal: React.FC<AuthModalProps> = ({ onLogin, language, onLanguageChange }) => {
  const [view, setView] = useState<'login' | 'signup' | 'google-sim'>('login');
  ...
  const handleGoogleLoginStep = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!googleName.trim()) {
    ...
    setIsLoading(true);
    try {
      const user = await loginWithGoogle(googleName);
      onLogin(user);
    } catch (err) {
  ...
  // Google Simulation UI
  if (view === 'google-sim') {
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900 bg-opacity-90 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden flex flex-col items-center p-8 transition-all">
                ...
                <p className="text-sm text-slate-500 mb-6 text-center">Simulating Google Auth Provider</p>
                
                <form onSubmit={handleGoogleLoginStep} className="w-full space-y-4">
                    <div className="w-full">
                        <label className="block text-xs font-medium text-slate-600 mb-1 ml-1">Enter your name to continue</label>
                        <input 
                            type="text" 
                            value={googleName}
                            onChange={(e) => setGoogleName(e.target.value)}
                            className="w-full px-4 py-3 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    <div className="flex justify-between items-center pt-2">
                        ...
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="bg-blue-600 text-white px-6 py-2 rounded-md font-medium hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-70"
                        >
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3 px-4 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-lg shadow-md transition-colors flex items-center justify-center"
            >
